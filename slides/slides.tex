...
\begin{frame}{The Impedance Mismatch}
    ...
    \begin{block}{Partitioned Distributed Architecture}
        Data partitioned across nodes for scalability.
        Each node maintains local state for its partition.
        Queries for non-local partitions incur network latency: $t_{\text{network}} \gg t_{\text{local}}$.
    \end{block}

    \textbf{Problem:} Queries may require remote state access.
    Cross-partition operations dominate cost.

    \begin{block}{Critical Operations}
        \begin{itemize}
            \item \textbf{Membership Testing:} Determine $x \in S$ where $S$ spans multiple partitions. Cost: one round-trip if $x$ not local.

            \item \textbf{Cardinality Estimation:} Compute $|S|$ for partitioned dataset. Exact \texttt{COUNT(DISTINCT)} requires shuffle. Cost: $\Theta(n \cdot |x|)$.
        \end{itemize}
    \end{block}
