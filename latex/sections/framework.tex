...
Composability requires that oracle state be derivable from commutative operations applied to element representations. Such operations necessarily lose information about individual elements. The HyperLogLog sketch (\ref{def:hll-sketch}) achieves this by retaining only maximum observables in each register, discarding element identities (subsection~\ref{subsec:hyperloglog}). The register-wise maximum operation used for sketch union cannot distinguish which specific elements contributed to a register's current value. This information loss enables order-independent aggregation.
...
\begin{theorem}[Incompatibility of Idempotent Composability and Deletion] \label{thm:incompatibility}
    Let $\mathcal{D}$ be a probabilistic oracle satisfying composability (\ref{def:composability}) with an idempotent merge operation $\cup$. If $\mathcal{D}$ supports a deletion operation $\textsc{Delete}(x)$ that guarantees no false negatives for elements in the represented set, then $\mathcal{D}$ must maintain reversible representations of inserted elements.
\end{theorem}
...
A hierarchical-reduction pattern produces correct global aggregates if and only if the oracle satisfies \ref{def:composability}. The HyperLogLog analysis in \S\ref{subsec:hyperloglog} shows that the union of partial sketches is equivalent to a sketch constructed over the complete dataset. Without commutativity and associativity, the result depends on merge order. Without idempotency, duplicate transmissions corrupt the aggregate.
...
% Fully-dynamic single-partition workloads require mutability without composability. The Cuckoo Filter (\S\ref{subsec:cuckoo-filters}) serves this class: membership testing in partitioned key-value stores, filters for LSM-tree SSTables undergoing compaction, and Pending Interest Tables in named-data networking.

% Static or append-with-expiry multi-partition workloads require composability without mutability. The HyperLogLog sketch (\S\ref{subsec:hyperloglog}) serves this class: cardinality estimation in MapReduce batch jobs, real-time analytics over sliding windows, and multi-datacenter monitoring.
