...
% Distributed SQL engines that execute \texttt{GROUP BY} queries with cardinality aggregation, stream processing systems that compute distinct counts over windowed data, and real-time analytics platforms that maintain cardinality estimates across organizational hierarchies all require cardinality estimation oracles whose internal state can be combined through the operation specified in \ref{def:composability}. We now examine the realization of this property in the HyperLogLog sketch.
...
\begin{definition}[State Divergence \cite{Huang2016TowardHD}]
    Let $M_{current}$ be the current state of an HLL sketch and $M_{backup}$ be its most recent persisted state. The state divergence, $D(M_{current}, M_{backup})$, is a metric quantifying the difference between the two sketches.
\end{definition}
...
\begin{definition}[Bucket k-Anonymity \cite{Le2025ByzantineRF}]
    Let $B$ be a set of elements matching a query, drawn from a background population $A$. An HLL bucket is defined to be k-anonymous if the value it contains could have been generated by at least $k$ distinct elements from the background population $A$. An attacker who observes the sketch cannot determine with certainty which of the $k$ elements contributed to that bucket's value.
\end{definition}
