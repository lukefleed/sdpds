\subsection{Probabilistic Membership Testing}

We consider the dictionary problem, which consists of maintaining a dynamic set $S \subseteq \mathcal{U}$ and supporting membership queries. This capability is a fundamental primitive for a class of distributed protocols concerned with set reconciliation, the process of computing the symmetric difference between sets held on disparate nodes. In a large-scale distributed environment, classical deterministic solutions to the membership problem, such as hash tables, are often infeasible due to space requirements proportional to both the number of elements and their individual sizes, which for large objects or high-cardinality sets becomes architecturally infeasible in bandwidth-constrained distributed systems, and the communication overhead required for synchronization \cite{Liu2014ParBFAP}. Specifically, a hash table requires $\Theta(n \cdot |x|)$ space for $n$ elements of size $|x|$ bits, whereas a Bloom filter requires $\Theta(n \log(1/\varepsilon))$ space independent of element size. The architectural constraints of such systems necessitate data structures that achieve sublinear space complexity \cite{Chang2006BigtableAD}. Probabilistic data structures fulfill this requirement by trading a quantifiable measure of precision for substantial gains in space and communication efficiency. This approach enables the design of systems that prioritize performance and availability over strict consistency.
...
\subsection{Limitations in Dynamic Environments} \label{subsec:bloom-limitations}

The properties of the standard Bloom filter hold under two assumptions: the set $S$ is static after construction, and its cardinality $n$ is known when the filter is initialized. Dynamic environments violate these assumptions. In systems where elements are continuously inserted and deleted, or where the total number of elements is not known in advance, two fundamental limitations emerge that restrict the filter's applicability.
...
\subsubsection{Counting Bloom Filter} \label{subsubsec:counting-bloom}
...
\subsubsection{Scalable Bloom Filter} \label{subsubsec:scalable-bloom}
...
\subsection{Distributed Patterns}
...
\subsubsection{Temporal False Negatives} \label{subsubsec:temporal-fn}
