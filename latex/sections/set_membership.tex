...
To support element deletion, the filter's structure must be augmented to track the number of elements mapped to each position. This capability was introduced as a core component of a scalable web cache sharing protocol, where the dynamic nature of cached content required a mechanism for deletions. The solution replaces the bit array with an array of counters \cite{fan2000summarycache}.
...
The operations are redefined to act upon these counters. To insert an element, we increment the counters at the $k$ hash positions. To delete an element, we decrement the corresponding counters. A membership query returns true if and only if all corresponding counters are positive \cite{fan2000summarycache}. The primary trade-off is a space complexity increase by a factor of $c$ compared to a standard Bloom filter. The selection of counter size $c$ must be sufficient to prevent overflow; analysis has shown that 4-bit counters are sufficient for most applications \cite{fan2000summarycache, tarkoma2011theory}. The arithmetic properties of the CBF are particularly useful in distributed systems for tasks such as set reconciliation, where the difference between two sets can be computed by subtracting one filter's counter array from another \cite{guo2013set}.
