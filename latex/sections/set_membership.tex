...
Probabilistic data structures resolve this constraint by trading a quantifiable measure of precision for substantial gains in space and communication efficiency. A Bloom filter, the canonical instance of this class, requires $\Theta(n \log(1/\varepsilon))$ space independent of element size, where $\varepsilon$ denotes the target false positive rate. This space bound becomes advantageous for elements whose size significantly exceeds $\log(1/\varepsilon)$ bits, as the oracle occupies space comparable to or smaller than storing even one copy of the original elements in a lossless representation.
% We establish the properties of probabilistic membership oracles and analyze the specific constraints that distributed deployment imposes on their design.
...
\begin{definition}[One-Sided Error Membership Oracle]
    A data structure $\mathcal{D}$ representing a set $S$ is a one-sided error membership oracle with a false positive rate $\varepsilon$ if the following hold over the randomness in constructing $\mathcal{D}$ from $S$ and in processing queries: for any element $x \in S$, $\mathcal{D}.\textsc{Query}(x)$ returns true with probability 1, and for any element $y \notin S$, $\mathcal{D}.\textsc{Query}(y)$ returns true with probability at most $\varepsilon$.
\end{definition}

% By definition, the oracle never produces false negatives. The adoption of such a structure is not an ad-hoc optimization but a principled architectural strategy. It represents a deliberate trade-off where a system prioritizes lower latency over strict consistency.
By accepting a bounded and well-defined error probability, these algorithms enable a level of scalability and performance that would be unattainable with exact, deterministic methods \cite{Tarkoma2012TheoryAP, Chang2006BigtableAD}.

\subsubsection{The Standard Bloom Filter}

The Bloom filter is the canonical implementation of a one-sided error membership oracle \cite{Bloom1970SpacetimeTI}. It achieves its space efficiency by storing compact representations of elements, derived from hash functions, rather than the elements themselves.

\begin{definition}[Bloom Filter \cite{Bloom1970SpacetimeTI}]
    A Bloom filter for a set $S$ with expected cardinality $|S| \leq n$ from a universe $\mathcal{U}$ consists of a bit array $B$ of length $m$ initialized to all zeros, and a collection of $k$ independent hash functions $\mathcal{H} = \{h_1, \ldots, h_k\}$, where each $h_i : \mathcal{U} \to \{0, 1, \ldots, m-1\}$. The hash functions are pairwise independent and map elements uniformly over the index range.
\end{definition}
...
The probability of a false positive, $\varepsilon$, represent the probability that all $k$ bits for the queried element are 1.
...
\begin{corollary}[Space-Error Relationship \cite{Broder2004NetworkAO}]
    To achieve a target false positive rate $\varepsilon$ with an optimal number of hash functions, the required number of bits per element is
    $$m/n = -\frac{\ln \varepsilon}{(\ln 2)^2} \approx 1.44 \log_2(1/\varepsilon)$$
\end{corollary}
...
\subsection{The Constraint of State Synchronization} \label{subsec:bloom-limitations}

In a distributed system where compute and storage are separated by network boundaries, a membership oracle must maintain coherence with a dataset that evolves through insertion and deletion operations across multiple partitions. This requirement for state synchronization introduces constraints not present in single-node deployments where the oracle and the data it represents are co-located. An oracle that fails to synchronize with the dataset violates a key property: for all times $t > t_{\text{delete}}$ where element $x$ is deleted at time $t_{\text{delete}}$, the oracle must satisfy $P(\mathcal{D}.\textsc{Query}(x) = \text{true}) \leq \varepsilon$. We identify two failure modes that manifest when a membership oracle cannot adapt its internal state to reflect changes in the underlying dataset.
...
\subsubsection{Variants and Trade-offs}

We analyze two extensions to the standard Bloom filter that address the failure modes identified in \ref{subsec:bloom-limitations}. Each extension resolves one constraint but introduces a new limitation that restricts its applicability in distributed contexts.

The Counting Bloom Filter \cite{Fan1998SummaryCA} addresses deletion by replacing the bit array with an array of counters. Each counter tracks the number of elements mapped to its position. Insertion increments the relevant counters; deletion decrements them. A query returns true if all corresponding counters are positive. This mechanism permits element removal without introducing false negatives for other elements.

\begin{definition}[Counting Bloom Filter \cite{Fan1998SummaryCA}] \label{def:counting-bloom}
...
The structure supports deletion but incurs a space overhead proportional to the counter width $c$. Analysis \cite{Fan1998SummaryCA, Tarkoma2012TheoryAP} demonstrates that 4-bit counters suffice to prevent overflow for most workloads, yielding an approximately four-fold increase in memory consumption relative to a standard Bloom filter. The arithmetic properties of counter arrays enable set difference operations through counter subtraction, a capability useful for distributed set reconciliation protocols \cite{Guo2013SetRV}.

The Scalable Bloom Filter presented in \cite{Almeida2007ScalableBF}, on the other hand, addresses the problem of capacity exhaustion through a sequence of constituent filters with increasing sizes and decreasing error rates. When the active filter reaches a load threshold, a new, larger filter is appended to the sequence. Queries then must inspect all filters in the sequence.

\begin{definition}[Scalable Bloom Filter \cite{Almeida2007ScalableBF}] \label{def:scalable-bloom}
    A Scalable Bloom Filter (SBF) is an ordered sequence of $s$ Bloom filters, $B_1, B_2, \ldots, B_s$. Each filter $B_i$ is defined by its size $m_i$ and number of hash functions $k_i$. The sequence is constructed such that for $i > 1$, $m_i > m_{i-1}$ and the target false positive rate $\varepsilon_i < \varepsilon_{i-1}$.
\end{definition}
...
\subsubsection{Dynamic Mutability}

The analysis in \ref{subsec:bloom-limitations} exposes a fundamental requirement for membership oracles in distributed systems. An oracle that cannot delete elements accumulates stale state, forcing redundant operations that negate the performance benefits the oracle was designed to provide. An oracle that cannot scale capacity requires either over-provisioning or service interruption for reconstruction. The CBF (\ref{def:counting-bloom}) resolves deletion at the cost of substantial space overhead. The SBF (\ref{def:scalable-bloom}) resolves capacity at the cost of lost composability.
...
\begin{definition}[Dynamic Mutability] \label{def:mutability}
    Let $S_t$ denote the set of elements for which $\textsc{Insert}$ has been called but $\textsc{Delete}$ has not been subsequently called at time $t$. A probabilistic oracle $\mathcal{D}$ exhibits dynamic mutability if it satisfies the following:

    (1) $\textsc{Insert}(x)$ and $\textsc{Delete}(x)$ execute in $O(1)$ time.

    (2) Space complexity is $O(|S_t| \log(1/\varepsilon))$ for a target false positive rate $\varepsilon$, where the bound holds as a function of the current cardinality.

    (3) For all $y \in S_t$, we have $P(\mathcal{D}.\textsc{Query}(y) = \text{true}) = 1$ (no false negatives for elements currently in the set).

    (4) The operation $\textsc{Delete}(x)$ removes the contribution of $x$ such that subsequent queries for any $y \neq x$ with $y \in S_t$ are unaffected. If $\textsc{Delete}(x)$ is called when $x \notin S_t$, the operation is a no-op.
\end{definition}
...
\subsection{Cuckoo Filters} \label{subsec:cuckoo-filters}

The Cuckoo Filter, introduced in \cite{Fan2014CuckooFP}, satisfies the conditions of dynamic mutability (Definition~\ref{def:mutability}). Its delete operation executes in $O(1)$ time, requires no auxiliary counters, and does not introduce false negatives for other elements. This property derives from its partial-key hashing scheme (\S\ref{subsubsec:partial-key-hashing}), which enables the relocation and removal of an element's fingerprint using only the fingerprint itself, decoupling the operation from access to the original element.
...
\begin{definition}[Partial-Key Cuckoo Hashing \cite{Fan2014CuckooFP}]
    For an element $x \in \mathcal{U}$, let $f = \textsc{fingerprint}(x)$ denote a compact bit string representation of $x$. Let $\textsc{hash}: \mathcal{U} \to \{0, 1, \ldots, m-1\}$ denote a hash function mapping elements to bucket indices, and let $\textsc{hash}_f: \{0,1\}^* \to \{0, 1, \ldots, m-1\}$ denote a hash function on bit strings (fingerprints) to bucket indices. The two candidate bucket indices are computed as:
    \begin{equation*}
        i_1 = \textsc{hash}(x), \quad i_2 = i_1 \oplus \textsc{hash}_f(f)
    \end{equation*}
    where $\oplus$ denotes bitwise XOR. This allows recovery of $i_1$ given $i_2$ and $f$ via the same equation:
    $$i_1 = i_2 \oplus \textsc{hash}_f(f)$$
    This symmetry decouples relocation from full-key access, enabling fingerprints to traverse network boundaries without requiring access to the original element.
\end{definition}
...
\begin{definition}[Cuckoo Filter Operations \cite{Fan2014CuckooFP}]
    ...
    \emph{Insert}$(x)$: Compute $i_1, i_2$. If either bucket contains an empty entry, place $f$ there and complete. Otherwise, select one bucket randomly, displace its resident fingerprint $f'$, compute the alternate location for $f'$ using $i \gets i \oplus \textsc{hash}(f')$, and repeat the displacement process. If relocations exceed a configurable threshold (typically 500 to 1000, chosen to balance insertion success probability and latency), insertion fails and a capacity increase is required.

    \emph{Delete}$(x)$: Compute $i_1, i_2$. If $f$ exists in either bucket, remove one copy; otherwise return failure. Deletion executes in constant time.

    \begin{remark}
        If two distinct elements $x$ and $y$ produce the same fingerprint $f$ and share a candidate bucket, deleting $x$ may erroneously remove the representation of $y$, introducing a false negative for $y$. This occurs with probability $O(1/2^f)$, which is typically negligible for fingerprints of sufficient bit length. The fingerprint size $f$ must be chosen such that $2^f \gg 1/\varepsilon$, where $\varepsilon$ is the target false positive rate.
    \end{remark}
\end{definition}
...
\begin{proposition}[False Positive Bound \cite{Fan2014CuckooFP}]
...
From this inequality, we can derive the minimum fingerprint size $f$ required to achieve a target false positive probability $\varepsilon$:
\begin{equation*}
    f \geq \lceil \log_2(1/\varepsilon) + \log_2(2b) \rceil
\end{equation*}
For a configuration with $b=4$ and a target $\varepsilon=0.01$ (a 1\% false positive rate), this relationship requires a fingerprint size of at least $f \geq \lceil \log_2(100) + \log_2(8) \rceil = \lceil 6.64 + 3 \rceil = 10$ bits.
...
The Dynamic Cuckoo Filter (DCF), introduced by Chen et al. \cite{Chen2017TheDC}, presents a systematic solution to this scaling problem. Instead of rebuilding a full filter, the DCF architecture allocates a new, empty filter and appends it to an ordered sequence of filters. All new insertions are directed to the most recently appended filter.

\begin{definition}[Dynamic Cuckoo Filter \cite{Chen2017TheDC}] \label{def:dynamic-cuckoo}
...
The linear degradation in query performance of the Dynamic Cuckoo Filter arises from its temporal organization. We observe that distributed systems have already addressed analogous scaling challenges for data partitioning. Architectures such as Apache Cassandra \cite{Lakshman2010CassandraAD} and Amazon DynamoDB \cite{Lakshman2010CassandraAD} employ consistent hashing to distribute their keyspace. In these systems, nodes are assigned token ranges on a virtual hash ring. An element's hash value deterministically maps it to a specific point on the ring, and thus to a responsible node. The addition or removal of a node requires remapping only a localized subset of the keyspace, preserving a stable and deterministic mapping for the majority of data. Redis Cluster \cite{redis-cluster-spec} achieves a similar outcome using a hash slot partitioning scheme, where a fixed number of slots are distributed among nodes and can be migrated individually to scale the cluster.
...
\begin{definition}[Index-Independent Cuckoo Filter \cite{Luo2021ACC}] \label{def:I2CF}
...
The core property of the I2CF is that bucket identifiers depend only on the element's hash value, not on the total number of buckets $m$ in the system. Capacity expansion is achieved by adding a new bucket to a chosen position on the ring. This operation necessitates relocating only those elements whose hash values fall within the range immediately preceding the new bucket's position. For a system with $n$ items distributed across $m$ buckets, a rebalancing event affects an expected $O(n/m)$ items, a small fraction of the total dataset.

This design decouples capacity growth from query complexity, resolving the conflict present in the DCF (\ref{def:dynamic-cuckoo})

\begin{theorem}[I2CF Query Complexity]
    For an I2CF with $m$ buckets organized on a consistent hash ring maintained as a hash table indexed by ring positions, a membership query for an element $x$ executes the following: (1) hash computations to determine ring positions in $O(1)$ time, (2) a hash table lookup to identify candidate buckets in $O(1)$ expected time, (3) inspection of at most $2b$ entries within the two candidate buckets. The total query complexity is therefore $O(1)$ in expectation, independent of the total number of items $n$ and the total number of buckets $m$. The auxiliary index incurs $O(m)$ space overhead, which is proportional to the number of buckets.
\end{theorem}
...
We now observe that the value of the I2CF (\ref{def:I2CF}) is most fully realized when its partitioning scheme is aligned with that of the host distributed system. Many distributed key-value stores partition their keyspace across a set of nodes using a consistent hash ring. In such systems, each node assumes responsibility for one or more token ranges on the ring.

We apply this same partitioning logic to the filter's buckets, an approach whose efficacy is demonstrated by Luo et al. in their work on distributed filters over Redis Cluster \cite{Luo2018DistributedDC}.
...
% This architecture instantiates the co-partitioning pattern that we will identify in the framework (\S~\ref{subsubsec:mutability-copartition}). The pattern emerges as a consequence of dynamic mutability:
...
\begin{theorem}[Co-Partitioned Query Latency] \label{thm:co-partitioned-latency}
    In a co-partitioned I2CF architecture of $m$ nodes, assume that (1) queries are uniformly distributed over the keyspace, (2) network latency is constant at $t_{\text{network}}$, and (3) local memory access time is constant at $t_{\text{local}}$. Under these assumptions, the expected query latency $\mathbb{E}[L_q]$ is given by:
    \begin{equation*}
        \mathbb{E}[L_q] = \frac{1}{m} t_{\text{local}} + \left(1 - \frac{1}{m}\right) t_{\text{network}}
    \end{equation*}
    where $t_{\text{local}}$ represents local bucket access time and $t_{\text{network}}$ represents network round-trip latency. This latency bound is independent of the total dataset size $n$.
\end{theorem}
...
This architecture allows the system to increase its total capacity instantly by adding a new, untapped I2CF. However, this \emph{scale-out} mechanism, if left unmanaged, structurally resembles the chaining model of the DCF (\ref{def:dynamic-cuckoo}). A query must probe every instance in the collection, leading to a performance dependency on the number of instances, $s$.

\begin{proposition}[CCF Unmanaged Query Complexity]
    In a CCF where new, fixed-capacity I2CF instances of constant capacity $C$ are added sequentially without consolidation, the number of instances $s$ grows linearly with the total number of items $n$. Specifically, $s = \lceil n/C \rceil$, which gives $s = \Theta(n)$ as $n \to \infty$ when $C$ is held constant. Since a query must probe every instance in the collection in the worst case, the worst-case query complexity is therefore $O(s) = O(n)$.
\end{proposition}

\begin{proof}
    Let $n$ be the total number of items and let each I2CF instance have an effective capacity of $C$ items held constant (e.g., $C = m \cdot b \cdot \alpha$, where $m$ is the number of buckets, $b$ is the entries per bucket, and $\alpha$ is the target load factor). In an unmanaged growth model with fixed per-instance capacity, a new instance is added every time the system has stored an additional $C$ items. The number of instances $s$ required to store $n$ items is therefore $s = \lceil n/C \rceil$. As $n \to \infty$, this gives $s = \Theta(n)$. Since a query must inspect all $s$ instances in the worst case, the query complexity is $O(s) = O(n)$.
\end{proof}
...
\begin{definition}[Optimistic Cuckoo Hashing \cite{Fan2013MemC3CA}]
    A writer maintains a version counter for each bucket. Before initiating relocations, it identifies a valid displacement path (e.g., $a \to b \to c \to \text{empty}$) without acquiring locks. The relocations are then executed in reverse order along the path: the element in the bucket preceding the empty slot is moved first, then the element preceding that, and so on until all elements have been relocated. Each move increments the version counter of the involved bucket. This reverse-order execution ensures that every element being relocated remains accessible in at least one of its two candidate buckets at all times. Specifically, if element $a$ is moved from bucket $B_1$ to bucket $B_2$, it is only after bucket $B_2$ has been vacated by moving its resident element to its alternate location, so when $a$ enters $B_2$, a reader examining either $B_1$ or $B_2$ will find $a$ before or after the move. A reader records the versions of its two candidate buckets before inspection and re-checks them afterward. If any version has changed, the reader discards its result and retries the operation.
\end{definition}
...
\begin{proposition}[Cuckoo Filter Replication Convergence]
    In an eventually consistent system with replicated Cuckoo Filters, let an element $x$ be inserted at time $t_i$ and deleted at time $t_d > t_i$. For any replica $r$, let the insertion be applied at time $\tau_i \geq t_i$ and the deletion be applied at time $\tau_d \geq t_d$. For all times $t > \tau_d$, we have $P(\mathcal{D}.\textsc{Query}(x) = \text{true}) = 0$ on replica $r$, since the deletion operation deterministically removes the fingerprint of $x$ from both candidate buckets. In contrast, a standard Bloom filter provides no mechanism to remove bits set during insertion, so it will continue to return a positive result for a deleted element indefinitely.
\end{proposition}
...
In named-data networking (NDN), we examine the Pending Interest Table (PIT), which tracks outstanding content requests. The DiCuPIT system \cite{Mahmoudi2023DiCuPITDC} utilizes distributed Cuckoo Filters to replace traditional Bloom filter-based implementations. This architectural change yields a 36\% reduction in lookup time and a 31\% reduction in memory consumption compared to Bloom filter baselines. The primary advantage is the ability to correctly and efficiently delete PIT entries as their corresponding data responses arrive.
...
\begin{definition}[Distributed Bloom Filter \cite{Ramabaja2019TheDB}]
...
\end{proposition}
\begin{proof}
    Let $\varepsilon = (1 - e^{-kn/m})^k$ be the false positive rate for a single filter. A system-wide false positive for a query from node $n_j$ regarding an element $y$ occurs if all $N-1$ peers it communicates with independently return a false positive. Let $E_i$ be the event of a false positive from peer $n_i$. The DBF protocol's use of unique seeds $s_{ji}$ for each peer interaction ensures that the hash function families $\mathcal{H}_{ji}$ are statistically independent for each peer $i$. Consequently, the events $E_i$ are independent. The probability of a joint failure is the product of their individual probabilities:
    $$P(\bigcap_{i=1}^{N-1} E_i) = \prod_{i=1}^{N-1} P(E_i) = \varepsilon^{N-1}$$
\end{proof}
...
The protocol also does not provide composability in the sense that will be formalized in Section \ref{sec:cardinality}. In that context, composability refers to the ability to merge two oracles constructed from different data partitions through a single, order-independent operation such that the merged oracle is equivalent to one constructed from the combined dataset. The DBF protocol does not satisfy this property because filters constructed with different node-specific hash seeds cannot be combined through a bitwise OR operation to produce a valid merged filter. Instead, the protocol achieves eventual consistency through iterative pairwise reconciliation, where each interaction uses node-specific hash functions.
